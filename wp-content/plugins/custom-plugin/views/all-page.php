<h2>AJAX</h2>
<hr>
<small><?=do_shortcode("[custom-plugin]")?></small>
<?php

?>
<form action="#" id="form_custom_add_outherPage">
	<table class="form-table">
		<tbody>
		<tr>
			<th scope="row"><label for="input_name">Name for outrhe page:</label></th>
			<td><input required name="input_name" type="text" id="input_name" value="" class="regular-text"></td>
		</tr>
		<tr>
			<th scope="row"><label for="input_email">E-mail of outher page: </label></th>
			<td><input required name="input_email" type="text" id="input_email" value="" class="regular-text"></td>
		</tr>
		<tr>
			<th></th>
			<td>
				<button type="submit" class="button button-primary">Salve</button>
			</td>
		</tr>

		</tbody>
	</table>
</form>

